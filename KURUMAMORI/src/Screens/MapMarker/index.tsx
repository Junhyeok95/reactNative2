    ...
    return (
      <SaveContainer>
        <TouchableOpacity
          style={
            index < 2
              ? {borderColor: '#00F', borderWidth: 3}
              : index < 4
              ? {borderColor: '#0AA', borderWidth: 3}
              : index < 6
              ? {borderColor: '#CA7', borderWidth: 3}
            ...
            if (item.DrivingMarker != undefined) {
              ...
              if (item.DrivingMarker && item.DrivingMarker.length > 0) {
                for (let i = 0; i < item.DrivingMarker.length; i++) {
                  ...
                  if (item.DrivingMarker[i].bool_sudden_stop)
                    _suddenStopMarker++;
                  if (item.DrivingMarker[i].bool_sudden_acceleration)
                    _suddenAccelerationMarker++;
                }
              }
            }
          ...
          <Save>
            <Icon2 name="map-clock-outline" color={'#000000'} size={40} />
          </Save>
          <SaveName numberOfLines={1}>
            {new Date(item.endTime).getMonth() +
              1 +
              ' / ' +
  ...
  return (
    <>
      <MapView
        ...
        showsIndoors={true}
        camera={camera}>
        {poly >= 0 && drivingSaveDataArr != undefined && (
          <Polyline
        ...
        {poly >= 0 &&
          ...
          drivingSaveDataArr[poly].DrivingMarker.map(
            (markerLocation: ILocation, index: number) => (
              <Marker
